...
reactive_vals <- reactiveValues(data = ecyp_db)
```
...
```{r}
textInput("enter_number", label = h4("Enter your number"), value = "", placeholder = NULL)

selectInput("variable", h4("Choose which field to update Info:"), colnames(ecyp_db)[-2])

renderUI({
  field_text <- names(variable_list)[[which(variable_list == input$variable)]]
  basicPage(
    textInput("new_value", label = h4("Enter the new value for ", field_text), placeholder = NULL), 
...
renderDataTable({dplyr::filter(reactive_vals$data, Unique_Number == input$enter_number)})
```
