...
```{r}
# Set up the variable list based on this but don't show the user ID or Last Updated as options to edit
...
output$UI <- renderUI({
  basicPage(
    ...
    textInput("new_value", label = h4("Enter the new value for ", input$variable), value = NULL),
  ...
  if (input$submit > 0) {
    ...
    output$UI <- renderUI({
      basicPage(
        ...
        textInput("new_value", label = h4("Enter the new value for ", input$variable), value = NULL),
        actionButton("submit", "Update Info", icon("fas fa-sync")),
        verbatimTextOutput(paste0("'", input$variable, "'", " was updated."))
      )
    })
  }
})
