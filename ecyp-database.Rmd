...
```{r}
# token <- gargle::token_fetch(scopes = "https://www.googleapis.com/auth/spreadsheets",
...
ecyp_update <- reactive({
  if (!is.null(input$submit) && input$submit > 0) {
    ...
    system(paste("Rscript update-googlesheet.R --new_value", input$new_value))
